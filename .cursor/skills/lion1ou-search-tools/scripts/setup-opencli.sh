#!/usr/bin/env bash
# ============================================================================
# opencli 安装脚本（lion1ou-search-tools）
# 适用于：Ubuntu 22.04+ / Debian 12+（需要 Node.js 18+）
# 用途：安装 opencli CLI，下载 Browser Bridge 插件，重启浏览器加载插件
# 使用：在 lion1ou-search-tools 目录运行 bash scripts/setup-opencli.sh
...
OPENCLI_EXTENSION_DIR="${OPENCLI_EXTENSION_DIR:-${OPENCLI_HOME}/opencli-extension}" # Browser Bridge 插件存放目录
...
download_browser_bridge() {
    ...
    local download_url
    local tmp_zip
    tmp_zip=$(mktemp "${TMP_DIR}/opencli-extension-XXXXXX.zip")
...
reload_browser_process() {
    ...
    # 后台启动，重定向输出到日志文件
    local log_file
    log_file=$(mktemp "${TMP_DIR}/opencli-chrome-restart-XXXXXX.log")
...
main() {
    echo ""
    echo "=== opencli 安装（lion1ou-search-tools） ==="
    echo ""
