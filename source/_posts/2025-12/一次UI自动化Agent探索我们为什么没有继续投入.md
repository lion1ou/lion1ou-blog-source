...
updated: 2026-09-22 16:00:00
toc: true
comments: true
categories: 杂记
tags: [Agent, Agent Harness, 复盘, 自动化测试, AI 工程化]
photos:
permalink: 2025-12/ui-automation-agent-exploration/
description: 我和测试团队做了一次移动端 UI 自动化 Agent 探索。模型只负责看懂页面和给出下一步，计划、循环调度、云真机设备会话、按角色供给上下文、两路评估和终止条件全部由自研 Harness 承担。技术闭环跑通后，我们结合投入成本、当时的模型能力和频繁中断的执行流程，判断短期内很难获得明显收益，于是停止追加投入，并把可复用成果补进现有测试平台。
---
...
立项时的假设是：在有需求文档或者冒烟用例的前提下，Agent 可以生成可执行的测试步骤，然后在真实设备上根据环境反馈自己把回归跑完。业务侧的目标定为覆盖 50% 无测试资源的需求。为什么按探索项目来做？模型能力当时还在快速变化，我们前端团队对测试这个专业的理解也有限。大家先用小投入跑通链路，再拿真实需求判断要不要加大投入。回头看，探索方向没有问题，欠缺的是立项时，对于实际测试场景的调研不够全面，低估了难度并高估了模型的能力。

## 整体是怎么设计的

先说分工。模型在这套系统里只做两件事：看懂当前页面，给出下一步动作和结果判断。其余全部由我们自己写的运行时承担：用例怎么变成计划、循环怎么转、动作怎么送到云真机、每一步给模型看什么、一步算不算成功、什么时候停、什么时候交给人。模型之外的这一整层，业界叫 Agent Harness。下面按这个分工来讲：计划环节、四角色循环、循环外的调度器、设备会话、报告与知识回流，最后是模型与知识库。

![模型之外的六层：计划、循环、环境、上下文、知识、评估，交人是正常出口](/images/2025-12/13-framework-harness-six-layers.png)
...
循环外面还有三层东西，当时花的精力并不比循环里少。这三层加上循环本身，就是这套 Harness 的主体。
...
后来再看，这套结构更接近分层的 Plan-and-Execute，按 Harness 的部件拆开是这样：计划层是人工确认后的用例和阶段计划，里面有路径、预期和阶段列表；循环层是四角色加调度器，一次只走当前能验证的一步，再根据页面反馈调整，步数预算、阶段超时和 Observation 指纹是硬终止条件；环境层是设备会话，连接、心跳、设备锁、串行执行和动作下发都在这里；上下文层决定每个角色每一步看到什么，后面「每一步都想把所有东西塞给模型」一节专门讲；知识层是三类知识库和确定性特例脚本；评估层单独由一个角色负责，又分成步骤评估和测试评估，结果连同每一步的截图和依据进报告，可以回放，也是知识回流的入口。模型写下的 Thought 只是推理过程。**真正能说明执行结果的，是截图、控件树、工具回执和断言。**
...
> 本节为 2026 年 8 月补充，「用 Harness 的分层再看」一小节为 9 月补充。下面引用的模型、框架和基准数据截至 2026 年 8 月。
...
在 2025 年下半年那个时点，用通用视觉模型和残缺的云真机控件树，我们把分层计划、四角色循环、设备会话、两路评估、调度终止条件、知识回流这套 Harness 跑通了，后来业界成熟方案里的对应物说明这套结构本身成立。项目留下了三样东西：一组用真实需求算出来的覆盖率、完成率和准确率，让停止投入的决定有据可查；一套异常分类和执行回放能力，已经进了现有平台；以及对「哪些问题模型升级能解决、哪些本来就是测试工程问题」的判断。这些是只看 Demo 得不到的。
