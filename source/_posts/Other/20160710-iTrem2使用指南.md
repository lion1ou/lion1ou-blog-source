...
具体配置详见另一篇文章：[oh my zsh 使用指南](http://lion1ou.win/2016/07/09/)
...
配置 `.bash_profile` 文件（mac 默认的配置文件，如果没有请自己新建一个）让终端每次启动的时候自动运行 Powerline 脚本，只需将下面代码`修改后`复制到你的 `bash_profile `文件里就可以啦：
...
安装完成后，将zsh设置成系统默认shell，以代替bash。 由于权限原因，先复制`/etc/shells`文件，然后在末尾添加/usr/local/bin/zsh，保存关闭后。替换源目录下的文件。再在终端中执行以下命令：
...
设置该主题，进入`vi ~/.zshrc`打开`.zshrc`文件，然后将ZSH_THEME后面的字段改为agnoster。
