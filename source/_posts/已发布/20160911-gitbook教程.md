...
1. README.md 是对书籍的简单介绍。
2. SUMMARY.md 是书籍的目录结构。
内容如下：
```
# SUMMARY
...
#### 使用 `gitbook serve` 编译预览书籍

书籍目录结构创建完成以后，就可以使用`gitbook serve`来编译和预览书籍了。`gitbook serve` 命令实际上会首先调用`gitbook build`编译书籍，完成以后会打开一个web服务器，监听在本地的 4000 端口。
...
可以提交修改：`git commit -asm "init book"`
