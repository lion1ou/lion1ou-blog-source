...
```
<!-- more -->
### 安装 [Powerline](http://powerline.readthedocs.io/en/latest/installation.html) （一个 vim 的插件，随后有用）
* 确认系统 python 版本是 2.7
...
* 由于我还没有安装过 `pip` ， `python` 安裝套件的指令，执行以下命令安装：
...
安装最新的zsh， `brew install zsh` ，重新启动 zsh 即可见到效果
